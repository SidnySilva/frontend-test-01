...
export const Dashboard = () => {
  ...
  const findItemByInput = (value) => { //filter by typing at search bar
    setData(
       widget.filter((el) =>
  ...
  return (
    ...
    >
      ...
      <TreeView
        ...
        sx={{ height: 240, flexGrow: 1, width: "90vw" ,marginTop:3,".highcharts-credits":{display:'none'}}}
      >
        {data && data.map((el) => (
          <TreeItem
            key={`${el.id}`}
            nodeId={`${el.id}`}
            label={`${el.title.text}`}
            sx={{ border: "1px solid lightGray", '.Mui-selected,.MuiTreeItem-label,	.MuiTreeItem-content':{padding:0} }}
          >
            <Widgets options={el} />
          </TreeItem>
        ))}
